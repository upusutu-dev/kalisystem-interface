...
export interface DatabaseSync {
  ...
  syncManagerTags: (managerTags: ManagerTag[]) => Promise<void>;
...
export const createDatabaseSync = (): DatabaseSync => {
  return {
    ...
    syncManagerTags: async (managerTags: ManagerTag[]) => {
      try {
        for (const tag of managerTags) {
          ...
          if (error) console.error('Error syncing manager tag:', error);
        }
      } catch (error) {
        console.error('Failed to sync manager tags:', error);
      }
    },
    ...
    loadFromDatabase: async () => {
      try {
        const [
          ...
          supabase.from('manager_tags').select('*'),
        ...
        const managerTags: ManagerTag[] = (managerTagsResult.data || []).map(row => ({
          id: row.id,
