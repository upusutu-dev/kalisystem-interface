...
export class SupabaseSync {
  ...
  static async syncManagerTags(managerTags: ManagerTag[]): Promise<void> {
    if (this.syncInProgress) return;
    this.syncInProgress = true;
  ...
  static async getManagerTags(): Promise<ManagerTag[]> {
    const { data, error } = await supabase
      .from('manager_tags')
      .select('*')
      .order('username');
  ...
  static async deleteManagerTag(tagId: string): Promise<void> {
    const { error } = await supabase
      .from('manager_tags')
      .delete()
      .eq('id', tagId);
